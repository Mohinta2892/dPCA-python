...
% keyboard
X = Xfull(:,:)';
% Xcen = bsxfun(@minus, X, 0);
% Xcen=bsxfun(@rdivide, X, std(X,0));
Xcen = bsxfun(@minus, X, mean(X));
% XfullCen = bsxfun(@minus, Xfull, 0);

XfullCen = bsxfun(@minus, Xfull, mean(X)');
% XfullCen=bsxfun(@rdivide, Xfull, std(X,0)');
...
if ~isempty(options.whichMarg) && ...
   length(unique(options.whichMarg)) <= 5 && length(unique(options.whichMarg)) > 1
